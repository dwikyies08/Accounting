    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <div class="mt-5">
                        <h4 class="card-title float-left mt-2">Data Cluster</h4>
                    </div>
                </div>
            </div>
        </div>
...
<<<<<<< HEAD
                <div class="card rounded-default p-3 bg-dark text-white">
                    <form method="GET" action="{{ route('cluster/list/page') }}">
...
=======
                <div class="card rounded-default p-3 filterBox text-white">
                    <form method="GET" action="{{ route('cluster/list/page') }}">
                        <div class="form-group">
                            <label>Pencarian</label>
                            <input type="text" name="nama_cluster" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Nama Cluster" value="{{ request('nama_cluster') }}">
...
>>>>>>> 8aec55e85e9d6a9b53b74c0e47ea6990ac3bf94a
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover table-center mb-0" id="ClusterList">
                ...
                <div class="page-header">
                    <div class="mb-15 row align-items-center">
                        <div class="col">
                            <a href="{{ route('cluster/add/new') }}" class="btn btn-primary float-left veiwbutton"><i class="fas fa-plus mr-2"></i>Tambah</a>
...
@section('script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#ClusterList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-cluster-data') }}",
                    data: function(d) {
            ...
            $('#select_all').on('click', function() {
                $('.cluster_checkbox').prop('checked', this.checked);
            });

            $('#ClusterList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.cluster_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
            ...
            $('#ClusterList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/cluster/edit/" + data.id;
                    }
            });
        });
    </script>
