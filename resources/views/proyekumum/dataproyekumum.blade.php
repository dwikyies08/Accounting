    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <div class="mt-5">
                        <h4 class="card-title float-left mt-2">Data Proyek UMUM</h4>
                    </div>
                </div>
            </div>
        </div>
...
<<<<<<< HEAD
                <div class="card rounded-default p-3 bg-dark text-white">
                    <form method="GET" action="{{ route('proyekumum/list/page') }}">
                        <div class="form-group">
                            <label>Pencarian</label>
                            <input type="text" name="proyek_umum_id" class="form-control" onchange="this.form.submit()" placeholder="Cari berdasarkan ID" value="{{ request('proyek_umum_id') }}">
                        </div> 
                        <div class="form-group">
                ...
                <div class="card rounded-default p-3 filterBox text-white">
                    <form method="GET" action="{{ route('proyekumum/list/page') }}">
                        <div class="form-group mb-1">
                            <label>Pencarian</label>
                            <input type="text" name="proyek_umum_id" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Cari berdasarkan ID" value="{{ request('proyek_umum_id') }}">
                        </div> 
                        <div class="form-group mb-1">
                            <input type="text" name="nama_proyek" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Nama Proyek" value="{{ request('nama_proyek') }}">
                        </div>
                        <div class="form-group mb-1">
                            <label>Dihentikan</label><br>
...
>>>>>>> 8aec55e85e9d6a9b53b74c0e47ea6990ac3bf94a
                        <div class="table-responsive">
                            <table class="table table-hover table-striped table-bordered table-center mb-0" id="ProyekUmumList">
                ...
                <div class="page-header">
                    <div class="mb-15 row align-items-center">
                        <div class="col">
                            <a href="{{ route('proyekumum/add/new') }}" class="btn btn-primary float-left veiwbutton"><i class="fas fa-plus mr-2"></i>Tambah</a>
...
@section('script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#ProyekUmumList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-proyekumum-data') }}",
                    data: function(d) {
                        d.nama_proyek = $('input[name=nama_proyek]').val(),
                        d.proyek_umum_id = $('input[name=proyek_umum_id]').val(),
            ...
            $('#select_all').on('click', function() {
                $('.proyekumum_checkbox').prop('checked', this.checked);
            });

            $('#ProyekUmumList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.proyekumum_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
            ...
            $('#ProyekUmumList tbody').on('click', 'tr', function(e) {
                ...
                var data = table.row(this).data();
                    if (data) {
                        window.location.href = "/proyekumum/edit/" + data.id;
                    }
            });
        });
    </script>
