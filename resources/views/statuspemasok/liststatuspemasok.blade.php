    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <div class="mt-5">
                        <h4 class="card-title float-left mt-2">Status Pemasok</h4>
                    </div>
                </div>
            </div>
        </div>
...
<<<<<<< HEAD
                <div class="card rounded-default p-3 bg-dark text-white">
                    <div class="form-group">
                        ...
                        <input type="text" name="nama" class="form-control key-filter" placeholder="Nama Status">
                    </div>
                </div>
            </div>
                ...
                <div class="card rounded-default p-3 filterBox text-white">
                    <form method="GET" action="{{ route('statuspemasok/list/page') }}">
                        <div class="form-group mb-1">
                            <label for="nama">Pencarian</label>
                            <input type="text" name="nama" class="form-control form-control-sm" onchange="this.form.submit()" placeholder="Nama" value="{{ request('nama') }}">
                        </div>
                    </form>
                </div>
            </div>
...
>>>>>>> 8aec55e85e9d6a9b53b74c0e47ea6990ac3bf94a
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover table-center mb-0" id="StatusPemasokList">
                ...
                <div class="page-header">
                    <div class="mb-15 row align-items-center">
                        <div class="col">
                            <a href="{{ route('statuspemasok/add/new') }}" class="btn btn-primary float-left veiwbutton"><i class="fas fa-plus mr-2"></i>Tambah</a>
...
<script src="https://cdn.datatables.net/buttons/2.4.1/js/dataTables.buttons.min.js"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#StatusPemasokList').DataTable({
                processing: true,
                serverSide: true,
                ordering: true,
                searching: false,
                ajax: {
                    url: "{{ route('get-statuspemasok-data') }}",
                    data: function(d) {
...
>>>>>>> 8aec55e85e9d6a9b53b74c0e47ea6990ac3bf94a
            ...
            $('#select_all').on('click', function() {
                $('.status_pemasok_checkbox').prop('checked', this.checked);
            });

            $('#StatusPemasokList tbody').on('mouseenter', 'tr', function() {
                $(this).css('cursor', 'pointer');
            });

            $('#deleteSelected').on('click', function() {
                var selectedIds = $('.status_pemasok_checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
                ...
                if (selectedIds.length > 0) {
                    if (confirm('Apakah yakin ingin menghapus data yang dipilih?')) {
                        $.ajax({
                            url: "{{ route('statuspemasok/delete') }}",
                            type: "POST",
                            data: {
                                ids: selectedIds,
                                _token: "{{ csrf_token() }}"
            ...
            $('#StatusPemasokList tbody').on('click', 'tr', function(e) {
                ...
                if (data) {
                    window.location.href = "/statuspemasok/edit/" + data.id;
                }
            });
        });
    </script>
